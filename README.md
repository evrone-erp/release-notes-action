...
|---------------------|----------------------------------------------------------------------------------------------------------------|
| YANDEX_ORG_ID       | Идентификатор организации из **Администрирование → Организации** в Yandex Tracker                              |
| YANDEX_OAUTH2_TOKEN | OAuth-токен приложения Трекера с разрешением `tracker:read` (см. инструкцию ниже)                               |

**Как получить OAuth-токен для Yandex Tracker:**

1. Перейдите на [oauth.yandex.ru](https://oauth.yandex.ru) и создайте приложение (**Для доступа к API или отладки**).
2. Добавьте разрешение **Чтение из трекера (tracker:read)**.
3. Скопируйте **ClientID** приложения.
4. Откройте в браузере ссылку (подставьте свой ClientID):
   ```
   https://oauth.yandex.ru/authorize?response_type=token&client_id=<ClientID>
   ```
5. Войдите в аккаунт, от имени которого экшен будет обращаться к API, и скопируйте выданный токен в секрет `YANDEX_OAUTH2_TOKEN`.

> Токены Яндекс ID (из [yandex.ru/dev/id](https://yandex.ru/dev/id/doc/ru/access)) больше не подходят — нужен именно OAuth-токен приложения Трекера.
